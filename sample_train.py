...
class TrafficLightEnv:
    ...
    def encode_state(self):
        ...
        # Give a lot of weight to the signal status
        light_code = self.current_light * (10 ** 9)
        traffic_code = 0
    ...
    def step(self, action):
        ...
        traffic_passed = min(
            self.traffic[self.current_light // 2, self.current_light % 2], np.random.randint(1, 6))

        # Update the traffic array.
        self.traffic[self.current_light // 2,
                     self.current_light % 2] -= traffic_passed

        # Determine the number of vehicles remaining in the lane at the current signal phase
        traffic_remaining = self.traffic[self.current_light //
                                         2, self.current_light % 2]
    ...
    def compute_reward(self, traffic_passed, traffic_remaining):
        efficiency = traffic_passed / \
            (traffic_passed + traffic_remaining + 1)  # 避免除以零
...
# Maximum number of steps per turn (simulated for 30 minutes, one time step per second)
learning_steps_per_episode = 1800
