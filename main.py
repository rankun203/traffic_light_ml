...
def draw_lanes(road):
    ...
    # # Set the start points for approach and exit lanes
    # start_x = road["x"]
    # start_y = road["y"]

    # # Determine the direction and length of the lanes based on road orientation
    # vertical = True if road["approach_direction"] in [
    #     "north", "south"] else False
    # lane_length = road["length"] if vertical else road["width"]

    # # Draw approach lanes
    # for i, lane_name in enumerate(road["approach_lanes"]):
    #     if road["approach_direction"] == "north":
    #         l_start = (start_x, start_y)
    #         l_end = (start_x, start_y + lane_length)
    #         r_start = (start_x + lane_width, start_y)
    #         r_end = (start_x + lane_width, start_y + lane_length)

    #         pygame.draw.line(screen, LANE_LINE, l_start, l_end)
    #         pygame.draw.line(screen, LANE_LINE, r_start, r_end)
    #         draw_text(lane_name, start_x + 10, start_y + 10, WHITE)
    #         start_x += lane_width
    #     elif road["approach_direction"] == "south":
    #         l_start = (start_x, start_y)
    #         l_end = (start_x, start_y + lane_length)
    ...
    #     else:
    #         pygame.draw.line(screen, LANE_LINE, (start_x, start_y + i *
    #                          lane_width), (start_x + lane_length, start_y + i * lane_width))
    #         pygame.draw.line(screen, LANE_LINE, (start_x, start_y + (i + 1) *
    #                          lane_width), (start_x + lane_length, start_y + (i + 1) * lane_width))

    # # Draw divider rect
    # if road["divider_width"] > 0:
    #     if vertical:
    #         x_y_w_h = (start_x + 1, start_y,
    #                    road["divider_width"]-1, lane_length)
    #         pygame.draw.rect(screen, DIVIDER, x_y_w_h)
    #     else:
    #         pygame.draw.rect(screen, DIVIDER, (start_x, start_y + road["length"] / 2 - road["divider_width"] / 2,
    #                                            lane_length, road["divider_width"]))
    # start_x += road["divider_width"]

    # # Draw exit lanes
    # # exit_start = (len(road["approach_lanes"]) -
    # #               len(road["exit_lanes"])) * lane_width / 2
    # for i, lane_name in enumerate(road["exit_lanes"]):
    #     if vertical:
    #         start_x += i * lane_width
    #         pygame.draw.line(screen, LANE_LINE, (start_x, start_y),
    #                          (start_x, start_y + lane_length))
    #         pygame.draw.line(screen, LANE_LINE, (start_x + lane_width, start_y),
    #                          (start_x + lane_width, start_y + lane_length))
    #     else:
    #         y_offset = start_y + start_x + i * lane_width
    #         pygame.draw.line(screen, LANE_LINE, (start_x, y_offset),
    #                          (start_x + lane_length, y_offset))
    #         pygame.draw.line(screen, LANE_LINE, (start_x, y_offset +
    #                          lane_width), (start_x + lane_length, y_offset + lane_width))
